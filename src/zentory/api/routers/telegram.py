...
ActionResultServiceDependency = Annotated[ActionResultService, Depends(get_action_result_service)]
...
async def telegram_webhook(
    ...
    action_result_service: ActionResultServiceDependency,
) -> dict[str, Any]:
    ...
    try:
        ...
        if command.event_type in {"action_result", "learning"}:
            return await _handle_learning_command(
                command, telegram_client, formatter, action_result_service
            )
...
async def _handle_learning_command(
    command: Any,
    telegram_client: TelegramClient,
    formatter: TelegramFormatter,
    service: ActionResultService,
) -> dict[str, Any]:
    if command.event_type == "action_result":
        result = service.get_action(command.action_id or "")
        result_payload = result.model_dump(mode="json") if result is not None else None
        text = formatter.format_learning_action_result(result_payload)
        sent = await telegram_client.send_telegram_message(command.chat_id, text)
        return {
            "mock": True,
            "status": "ok" if result is not None else "not_found",
            "event_type": command.event_type,
            "proposed_actions": [],
            "learning_result": result_payload,
            "telegram_messages": [sent],
        }

    actions = [item.model_dump(mode="json") for item in service.list_by_sku(command.sku or "")]
    text = formatter.format_learning_sku(command.sku or "", actions)
    sent = await telegram_client.send_telegram_message(command.chat_id, text)
    return {
        "mock": True,
        "status": "ok",
