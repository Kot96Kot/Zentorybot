...
class WildberriesClient:
    ...
    async def fetch_orders(self) -> list[dict[str, Any]]:
        return [{"mock": True, "marketplace": self.marketplace, "orders": 3}]

    async def fetch_stocks(self) -> list[dict[str, Any]]:
        return [{"mock": True, "sku": "WB-MOCK-1", "stock": 42}]

    async def fetch_prices(self) -> list[dict[str, Any]]:
        return [{"mock": True, "sku": "WB-MOCK-1", "price": 1990}]
    ...
    async def fetch_ads_stats(self) -> list[dict[str, Any]]:
        return [{"mock": True, "drr": 0.12, "ctr": 0.04}]
