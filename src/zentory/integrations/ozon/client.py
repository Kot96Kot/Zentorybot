...
class OzonClient:
    ...
    async def fetch_orders(self) -> list[dict[str, Any]]:
        return [{"mock": True, "marketplace": self.marketplace, "orders": 2}]

    async def fetch_stocks(self) -> list[dict[str, Any]]:
        return [{"mock": True, "sku": "OZON-MOCK-1", "stock": 31}]

    async def fetch_prices(self) -> list[dict[str, Any]]:
        return [{"mock": True, "sku": "OZON-MOCK-1", "price": 2090}]

    async def fetch_reviews(self) -> list[dict[str, Any]]:
        return [{"mock": True, "rating": 4, "text": "mock review"}]
    ...
    async def fetch_ads_stats(self) -> list[dict[str, Any]]:
        return [{"mock": True, "drr": 0.16, "ctr": 0.03}]
