    ...
    def check_abc(
        ...
        *,
        ...
        for item in input_items:
            ...
            if item.profit < 0:
                warnings.append(f"SKU {item.sku}: profit отрицательный, проверьте unit economics")
            if item.revenue < 0:
    ...
    def check_forecast(
        ...
        *,
        ...
        for forecast in report.items:
            ...
            if min(source.sales_qty_7d, source.sales_qty_14d, source.sales_qty_30d) < 0:
        ...
        for forecast in report.items:
            if min(
                forecast.avg_daily_sales_7d,
                forecast.avg_daily_sales_14d,
                forecast.avg_daily_sales_30d,
            ...
            if forecast.avg_daily_sales == 0 and forecast.stock_coverage_days is None:
                warnings.append(f"SKU {forecast.sku}: coverage не рассчитан без деления на 0")

        checked_rules_count += 1
        if any(not item.mock_mode for item in report.items):
            errors.append("Каждый forecast item должен явно иметь mock_mode=true")
