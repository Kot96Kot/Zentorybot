...
- Orchestrator и 16 agent-классов/доменных модулей.
...
## Функции менеджера, превращенные в агентов
...
12. Финансовая сверка — `FinanceCheckerAgent`.
13. ABC/forecast — `ForecastABCAgent`.
14. SKU Intelligence — `SKUAgent`.
15. Supply planning — `SupplyAgent`.
16. Content CTR — `ContentCTRAgent`.
...
- `GET /sku/{sku}/intelligence` — mock SKU Intelligence Card.
- `GET /analytics/abc` — ABC-analysis by revenue.
- `GET /analytics/forecast` — mock forecast bundle.
- `GET /supply/risks` — stock and supply risks.
- `GET /finance/check` — mock Finance Checker report.
- `POST /learning/action-result` — record post-action metrics for Learning Loop.
- `GET /learning/actions` — list Learning Loop records.
- `GET /learning/sku/{sku}` — list Learning Loop records for one SKU.
...
curl http://localhost:8000/finance/check
curl http://localhost:8000/learning/actions
...
Последняя локальная проверка: 2026-05-17.

- `ruff check .` — passed.
- `pytest` — passed: 189 collected, 186 passed, 3 xfailed.
- `python -m compileall src` — passed.

Для тестов в sandbox используется lightweight ASGI test client in `tests/conftest.py`, чтобы не зависеть от отсутствующего `httpx`; runtime-приложение остается обычным FastAPI app.
...
src/zentory/agents           # agent-классы, Orchestrator, team roles
...
- `docs/mvp_demo_checklist.md` — финальная проверка готовности к MVP demo: endpoints, Telegram UX, safety, mock-only limits and WB API next step.
