...
def test_forecast_abc_agent_returns_sku_forecast_card() -> None:
    ...
    assert card["forecast"]["avg_daily_sales_7d"] > 0
    assert card["forecast"]["avg_daily_sales_14d"] > 0
    assert card["forecast"]["avg_daily_sales_30d"] > 0
...
def test_forecast_abc_telegram_commands_are_supported() -> None:
    abc = TelegramCommandService().parse({"text": "/abc profit", "chat_id": "1"})
    forecast = TelegramCommandService().parse({"text": "/forecast", "chat_id": "1"})
    forecast_sku = TelegramCommandService().parse(
        {"text": "/forecast_sku ZNT-COS-001", "chat_id": "1"}
    )

    assert abc.event_type == "abc"
    assert abc.payload["metric"] == "profit"
    assert forecast.event_type == "forecast"
    assert forecast_sku.event_type == "forecast_sku"
    assert forecast_sku.payload["sku"] == "ZNT-COS-001"
