...
## MVP demo contracts verified on 2026-05-17

The demo contract for API responses is intentionally mock-first:

- response bodies use `mock`, `mock_mode`, or integration-specific `not_sent` markers;
- Action Center records include risk level, approval mode, status, payload, evidence and rollback metadata;
- Finance Checker records include status, issue type, affected SKU, expected/actual values, recommendation and `needs_human_check`;
- Learning Loop records include `action_id`, `agent`, `sku`, `recommendation`, `expected_effect`, `metric_before`, optional `metric_after`, `effect`, `success_score`, `conclusion`, `status` and `mock_mode`;
- SKU Intelligence cards remain deterministic mock cards suitable for dashboard/API demos.

The required MVP smoke endpoints are covered by `tests/unit/test_mvp_demo_readiness.py`:
...
Before connecting real WB/Ozon/Yandex data, adapters must preserve these internal contracts and pass through `DataNormalizationService`.
